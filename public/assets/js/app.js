...
$(document).ready(function() {
	...
	if (localStorage&&localStorage.getItem('expand_left_menu')==0)
	...
	$('.minifyme').click(function(e) {
		...
		localStorage.setItem('expand_left_menu', ($('body').hasClass('minified') ? 0 : 1));
		e.preventDefault();
	});
	...
	// LOGOUT BUTTON
	$('#logout-dep a').click(function(e) {
		//get the link
		$.loginURL = $(this).attr('href');

		// ask verification
		$.SmartMessageBox({
			title : "<i class='fa fa-sign-out txt-color-orangeDark'></i> 安全退出 <span class='txt-color-orangeDark'><strong>" + $('#show-shortcut').text() + "</strong></span> ?",
			content : "你可以在安全退出后再关闭此浏览器窗口",
			buttons : '[否][是]'
